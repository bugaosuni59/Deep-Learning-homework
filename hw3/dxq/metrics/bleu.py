...
def cal_blue_score(pred, label):
    ...
    for i in range(B):
        trans.append(pred[:, i])
        refs.append([label[:, i]])
    return compute_bleu(refs, trans)
...
if __name__ == "__main__":
    ...
    label[0] = np.array(reference)+1
    label[-1] = np.array(reference)-1

    print(pred.shape, label.shape)

    print(sentence_bleu([reference], hypothesis))
    print(compute_bleu([[reference]], [hypothesis, hypothesis]))
    print(cal_blue_score(pred.transpose((1,0)), label.transpose((1,0))))