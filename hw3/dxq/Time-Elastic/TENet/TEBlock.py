...
from utils.hyper import Hyperparams as hp
from utils.opts import args
from TENet.modules import *
...
class RelationModuleMultiScale(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        for scaleID in range(1, len(self.scales)):
            ...
            if args.ada_type == "type_two":
                low_feature = temp_2
                low_level_feature = low_feature.view((-1, low_feature.size(1) * low_feature.size(2)))
                low_level_result = self.final(low_level_feature)
            else:
                low_feature = temp_1 + temp_2 + temp_3
                low_level_feature = low_feature.view((-1, low_feature.size(1) * low_feature.size(2)))
                low_level_result = self.final(low_level_feature)
        ...
        if args.ada_type == "type_one":
            return act_all_result, act_feature, high_level_result, high_level_feature
        else:
            return act_all_result, act_feature, low_level_result, low_level_feature
